...
import { Award, ShieldCheck, CheckCircle2, Star, Download, ChevronLeft, CreditCard, Activity, Copy, Check } from 'lucide-react';
        ...
        <div className="section" style={{ paddingTop: '2rem' }}>
            <div className="container" style={{ maxWidth: '800px' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>Your official renter reputation score in the UAE.</p>
                </div>
                ...
                <div className="glass-card" style={{ padding: '3rem 2rem', textAlign: 'center', marginBottom: '2rem', position: 'relative', overflow: 'hidden' }}>
                    ...
                    {isPremium ? (
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'rgba(245,158,11,0.1)', borderRadius: 'var(--radius-full)', border: '1px solid rgba(245,158,11,0.3)' }}>
                            ...
                            <span style={{ fontWeight: 600, color: '#f59e0b' }}>Premium GCC Status Achieved</span>
                        </div>
                    ) : (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                    <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                        <div style={{ padding: '0.5rem', background: currentUser.isUaePassVerified ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.05)', borderRadius: 'var(--radius-md)', color: currentUser.isUaePassVerified ? 'var(--success)' : 'var(--text-muted)' }}>
                            <ShieldCheck size={24} />
                        </div>
                        <div>
                            <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>UAE PASS Identity KYC</div>
                            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>Government verified identity linked.</div>
                            {currentUser.isUaePassVerified ? <div style={{ fontSize: '0.75rem', color: 'var(--success)', marginTop: '0.5rem', fontWeight: 600 }}>+20 Points</div> : <div style={{ fontSize: '0.75rem', color: 'var(--warning)', marginTop: '0.5rem' }}>Action Required</div>}
                        </div>
                    </div>

                    <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                        <div style={{ padding: '0.5rem', background: 'rgba(34,197,94,0.1)', borderRadius: 'var(--radius-md)', color: 'var(--success)' }}>
                            <CheckCircle2 size={24} />
                        </div>
                        <div>
                            <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>No RERA Rental Disputes</div>
                            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>Clean record with the Rental Dispute Center.</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--success)', marginTop: '0.5rem', fontWeight: 600 }}>+30 Points</div>
                        </div>
                    </div>

                    <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                        <div style={{ padding: '0.5rem', background: currentUser.good_conduct_certificate?.payment_reliability === 'excellent' ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.05)', borderRadius: 'var(--radius-md)', color: currentUser.good_conduct_certificate?.payment_reliability === 'excellent' ? 'var(--success)' : 'var(--text-muted)' }}>
                            <CreditCard size={24} />
                        </div>
                        <div>
                            <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>On-Time Rent Payments</div>
                            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>100% on-time processing for the last 12 months.</div>
                            {currentUser.good_conduct_certificate?.payment_reliability === 'excellent' ? <div style={{ fontSize: '0.75rem', color: 'var(--success)', marginTop: '0.5rem', fontWeight: 600 }}>+25 Points</div> : <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>Insufficient Data</div>}
                        </div>
                    </div>
                    ...
                    <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                        ...
                        <div>
                            <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>Peer Reviews</div>
                            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: '0.375rem' }}>Verified roommate reviews for cleanliness and noise.</div>
                            <div style={{ display: 'flex', gap: '0.125rem', color: '#f59e0b' }}>
                                <Star size={12} fill="#f59e0b" />
                                <Star size={12} fill="#f59e0b" />
                                <Star size={12} fill="#f59e0b" />
                                <Star size={12} fill="#f59e0b" />
                                <Star size={12} fill="rgba(255,255,255,0.2)" />
                            </div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--success)', marginTop: '0.5rem', fontWeight: 600 }}>+10 Points</div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                    ...
                    >
                        ...
                        {copied ? 'Link Copied!' : 'Copy Verification Link for Landlords'}
                    </button>
                </div>
